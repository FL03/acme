...
Resolution options (Joe's call, or the first design session): (a) re-shape the workspace onto the spec's vocabulary (policy families as modules; the five primitives as a trait boundary with an in-memory simplicial backend until eryon exists), (b) keep the engine as a generic ingestion shell and build the policy layer beside it, (c) treat v0.4.x as the spec-alignment arc. Note that the ACME doc needs NO eryon runtime to start: the five primitives define a contract an in-memory store can satisfy for tests and evals.
...
2. **Spec/scaffold divergence** (§5 above): blocking for any implementation work.
...
- **Notion may be newer than the vault.** `claude/captures/notion-docs-reconciliation.md` records a Notion "Acme" page and an "Eryon — Technical Specification" page last edited 2026-05-31, flagged as likely fuller than the vault copies at that time, with an explicit hold: Joe picks the merge direction before any vault edit. The vault ACME spec compiled here may not be the latest text. Confirm with Joe before treating it as final; never overwrite vault notes.
